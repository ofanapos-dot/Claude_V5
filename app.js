...
function hitungKorelasi() {
  ...
  const tanggal = [], la = [], lb = [], pairsA = [], pairsB = [], anomali = [];
  ...
  for (let i = Math.max(0, idxMulai); i <= idxAkhir; i++) {
    ...
    if (valA[i] !== null && valA[i] !== undefined && valB[i] !== null && valB[i] !== undefined) {
      pairsA.push(valA[i]); pairsB.push(valB[i]);
      const beda = Math.abs(valA[i] - valB[i]);
      if (beda > 25) anomali.push({ tanggal: data.dates[i], a: valA[i], b: valB[i], beda: Math.round(beda * 10) / 10 });
    }
  }
  ...
  setHTML('korStatGrid', `
    <div class="stat-card"><div class="stat-label">Korelasi Harian</div><div class="stat-num">${isNaN(corr) ? '-' : corr.toFixed(3)}</div></div>
    <div class="stat-card"><div class="stat-label">Hari Beririsan</div><div class="stat-num">${pairsA.length}</div></div>
    <div class="stat-card"><div class="stat-label">${esc(namaA)}</div><div class="stat-num" style="font-size:0.95rem">rata² ${(pairsA.reduce((a,b)=>a+b,0)/pairsA.length).toFixed(1)} mm</div></div>
    <div class="stat-card"><div class="stat-label">${esc(namaB)}</div><div class="stat-num" style="font-size:0.95rem">rata² ${(pairsB.reduce((a,b)=>a+b,0)/pairsB.length).toFixed(1)} mm</div></div>
  `);
  ...
  state.chartKorLine = new Chart(el('chartKorLine'), {
    ...
    data: {
      labels: tanggal,
      datasets: [
        { label: namaA, data: la, borderColor: '#3b82f6', backgroundColor: 'transparent', spanGaps: true, tension: 0.15, pointRadius: 0 },
        { label: namaB, data: lb, borderColor: '#f59e0b', backgroundColor: 'transparent', spanGaps: true, tension: 0.15, pointRadius: 0 },
      ],
    },
  ...
  if (!anomali.length) {
    tbody.innerHTML = `<tr><td colspan="4" style="text-align:center;color:#94a3b8;padding:20px">Tidak ada hari dengan selisih &gt;25mm pada periode ini.</td></tr>`;
  } else {
